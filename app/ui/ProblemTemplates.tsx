...
export const difficultyBodyTemplate = (rowData: Problem) => {
  const severityMap: Record<
    Problem["difficulty"],
    { severity: "success" | "warning" | "danger"; icon: string; color: string }
  > = {
    Easy: {
      severity: "success",
...
export const dateBodyTemplate = (rowData: Problem) => (
...
export const actionBodyTemplate = (
  ...
  return (rowData: Problem) => {
    ...
    const deleteProblem = async () => {
      try {
        ...
        const updatedProblems = problems.filter(
          (problem) => problem.id !== rowData.id
        );
        setProblems(updatedProblems);
        setTotalItems(updatedProblems.length);
    ...
    const navigateToEdit = () =>
      router.push(`/dashboard/create-problem?edit=true&id=${rowData.id}`);
